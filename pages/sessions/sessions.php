<?php
/**
 * Learner Requests / Sessions — main center content only. Shared by TWO
 * distinct routes (see index.php's $routes map), rather than duplicating
 * this file:
 *   index.php?page=sessions           -> Sessions (Upcoming/Completed/Cancelled)
 *   index.php?page=learner-requests   -> Learner Requests (Mentor-only view)
 * index.php sets $sessionView ('sessions' | 'requests') before including
 * this file — that one variable is what makes the two routes visually and
 * functionally distinct while sharing one implementation, one right-sidebar
 * context (includes/right-sidebar.php's 'sessions' case) and one exact-match
 * active-nav slug each (index.php's $activeNav = $page, compared against
 * each nav item's own distinct route in includes/left-sidebar.php — already
 * correct, not touched here).
 *
 * This file holds ONLY page-specific main content — the header, left
 * sidebar, contextual right sidebar and footer come from index.php's shell.
 *
 * Tabs reuse Bootstrap's .nav-tabs (already retheme'd for this exact page,
 * see components.css); session/request cards reuse components/session-card.php
 * as-is. Accept/Reject/Cancel are wired by assets/js/pages/sessions.js;
 * Reject/Cancel reuse the one shared modals/delete-confirmation-modal.php,
 * Rate Mentor reuses the one shared modals/rating-modal.php — both via the
 * same contextual assets/js/core/modal.js mechanism used everywhere else.
 *
 * Frontend-only mock data throughout — no real accept/reject/cancel/rating
...
if ($sessionView === 'requests'):
    /**
     * Mentor-oriented, but frontend-only: opening this route in Learner
     * mode does not corrupt role state or break the shell — it just shows
     * the same mentor-facing content regardless of active role.
     */
    $requests = [
